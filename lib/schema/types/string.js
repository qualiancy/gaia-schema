...
TypeString.prototype._validate = function (value, spec) {
  ...
  assertions.enum(value, spec, assert);

  if ('regexp' === typeOf(spec.re)) {
    assert(spec.re.test(value),
      'Expected value to pass regular expression: ' + spec.re);
  }
...
TypeString.prototype._wrap = function(value, spec) {
  var res = value;

  if ('function' === typeOf(spec.wrap)) {
    res = spec.wrap(value, spec);
  }

  return res;
};

TypeString.prototype._unwrap = function(value, spec) {
  var res = value;

  if ('function' === typeOf(spec.unwrap)) {
    res = spec.unwrap(value, spec);
  }
