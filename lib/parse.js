...
var debug = require('sherlock')('gaia-schema:parse');
...
/*!
 * Determine if top-level spec has no paths and is therefor
 * a root only schema.
 *
 * @param {Object} top level spec
 * @return {Boolean} is root
 * @api private
 */

function isRoot(specs) {
  if ('string' === typeOf(specs)) return true;
  if ('function' === typeOf(specs)) return true;
  if ('object' == typeOf(specs) && specs.type) return true;
  return false;
}
